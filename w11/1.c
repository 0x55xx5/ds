...
while(rear<max&&val!= -1) {
        ...
        if(val==0) {
            ...
            front=(front+1)% max; printf("withdraw data [%d]\n", queue[front]);
            queue[front]=0;
        }

        else if(val!=-1&&rear<max) {
            if(rear+1==front || (rear==max-1&&front<=0)) { printf("FULL\n"); break; }
            rear=(rear+1)%max; queue[rear]=val;
        }
}
...
{
    while(front!=rear) {
    front=(front+1)%max; printf("[%d]", queue[front]); queue[front]=0;
    }
    printf("\n"); 
    return 0;
}
