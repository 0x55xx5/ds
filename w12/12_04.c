...
int main() {
    ...
    do {
        printf("enter new node:"); scanf("%d",&data);
        if (data == -1) break;
        if((search(ptr,data))!=NULL) printf("exist %d!\n",data);
        else { ptr=creat_tree(ptr,data); inorder(ptr); }
    } while (1);

    do {
        printf("enter del val:"); scanf("%d", &data);
        if (data == -1) break;
        if((search (ptr, data))!= NULL) { ptr=delete_n(ptr,data); inorder(ptr); } 
