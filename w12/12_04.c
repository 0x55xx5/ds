...
btree creat_tree (btree root, int val) {  
    ...
    if(root==NULL) { root=newnode; return root; }
        ...
        for (current=root; current!=NULL;) {
            backup=current; if(current->data > val) current=current->left; else current=current->right;
        }
...
btree search (btree ptr,int val) {
    ...
    while(1) {
        ...
        if(ptr->data==val) { printf("ser %d times\n",i); return ptr; }
        else if(ptr->data > val) ptr=ptr->left; else ptr=ptr->right;
        i++;
    }
}
...
if (r == NULL) return r;
else if (key < r->data) r->left = delete_n(r->left, key);
else if (key > r->data) r->right = delete_n(r->right, key); 
else {
// 節點只有一個或沒有子節點的情況
if (r->left == NULL) { btree temp = r->right; free(r); return temp; }
else if (r->right == NULL) { btree temp = r->left; free(r); return temp; }
// 節點有兩個子節點的情況
...
void inorder(btree ptr) { if(ptr!=NULL) { inorder(ptr->left); printf("[%2d] ",ptr->data); inorder(ptr->right); } }
int main() {
int i,data,arr[]={7,1,4,2,8,13,12,11,15,9,5}; btree ptr=NULL;
printf("orig \n"); for (i=0;i<11;i++) { ptr=creat_tree(ptr,arr[i]); printf("[%2d] ",arr[i]); } printf("\n");
do {
printf("enter new node::"); scanf("%d",&data); if (data == -1) break;
if((search(ptr,data))!=NULL) printf("exist %d!\n",data); else { ptr=creat_tree(ptr,data); inorder(ptr); }
} while (1);

do {
    printf("enter del val:"); scanf("%d", &data); if (data == -1) break;
    if((search (ptr, data))!= NULL) { ptr=delete_n(ptr,data); inorder(ptr); } else printf("not founded %d!\n", data); 
    } while (1);
