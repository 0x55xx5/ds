...
btree temp;
if (seq[index]==0 ||index >= size) return NULL;
else {
...
void preorder(btree node) {
    if (node != NULL ) { printf("%c",(char)node->value); preorder(node->left); preorder(node->right); }
}

void inorder(btree node) {
    if (node != NULL) { inorder(node->left); printf("%c",(char)node->value); inorder(node->right); }
}

void postorder(btree node) {
    if ( node != NULL ) { postorder(node->left); postorder(node->right); printf("%c",(char)node->value); }
}
...
int answer(btree node) {
    int first = 0, sec = 0;
    if (node->left == NULL && node->right== NULL ) return node->value-48;
    else {
        first = answer(node->left); sec = answer(node->right); return cond((char)node->value, first, sec);
    }
}
