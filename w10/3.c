...
int pop() { if (top != -1) return p1[top--]; else return -1; }
...
case '*': return (op1*op2); case '/': return (op1/op2);
case '^': for (i=1; i<=op2; i++) s*=op1; return s;
}
}
int main() {
int n,aa,opl, i; char c,fg; posfix[0]=0;
while (1) {
    char exp[100];
    int pos=0, op1,op2,s1=0,fir=0;
    printf("postfix statement--> "); gets(exp); if (exp[0] == '\0') break;
    while (exp[pos] != '\0' && exp[pos] != '\n') {
          if (isoperator(exp[pos])) { /* '+ - * /' */
             op2=pop(); op1=pop(); push(eval(exp[pos],op1,op2));
        }
        else
            if (exp[pos]==' ') {
                if ((fir == 0) && (s1 != 0)){ push(s1); s1=0; fir=1; }
            }
            else { s1=s1*10+exp[pos]-'0'; fir=0; }
        pos++;
    }
